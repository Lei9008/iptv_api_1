...
def match_channels(template_channels, all_channels):
    ...
    for online_category, online_channel_list in all_channels.items():
        for online_channel_name, _ in online_channel_list:
            # 去重优化：避免重复添加相同频道名称，提升匹配效率
            if online_channel_name not in all_online_channel_names:
...
def updateChannelUrlsM3U(channels, template_channels):
    # 更新频道URL到M3U和TXT文件中（已启用排序过滤，移除URL后缀逻辑）
            ...
            if category in channels:
                for channel_name in channel_list:
                    if channel_name in channels[category]:
                        # 优化1：提取所有URL并去重，避免重复写入
                        all_urls = list(set(channels[category][channel_name]))
                        # 优化2：启用sort_and_filter_urls，实现黑名单过滤和IP优先级排序
                        sorted_urls_ipv4 = sort_and_filter_urls(all_urls, written_urls_ipv4)
                        sorted_urls_ipv6 = sort_and_filter_urls(all_urls, written_urls_ipv6)

                        # 直接使用过滤排序后的URL，不再调用add_url_suffix添加后缀
